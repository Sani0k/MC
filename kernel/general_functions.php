...
function a_profiler($start_time) {
	...
	echo '<!--Время выполнения: '. round($end_time - $start_time, 5) . PHP_EOL . PHP_EOL;
	...
	if($db) {
		...
		if(count($db->list_queries) > 0) {
			foreach($db->list_queries AS $query_data) {
			    echo   $query_data['query'] . round($query_data['time'], 5) . PHP_EOL;
	...
	echo PHP_EOL . PHP_EOL;
	echo 'GET данные:'. PHP_EOL . print_r($_GET) . PHP_EOL . PHP_EOL;
	
	echo'POST данные:'. PHP_EOL .print_r($_POST) .'!-->';
